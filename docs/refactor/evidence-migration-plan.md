...
| **E-DOC-006** | 主控板双芯片操作与例程使用必读说明 | `4例程资料/主控板例程/0使用前必看说明/使用说明.docx` | `P01`: S1 (ESP32-U4WDH) 与 S3 (ESP32-S3) 芯片型号<br>`P02`: Type-C 无缝朝上与白色按键切换规程<br>`P03`: S3/S1 硬件复位按键空间排布<br>`P04`: S1 复位串口输出硬件注册码机制 | **LANDED** |
| **E-DOC-007** | 客户注册码获取与Arduino环境配置教程 | `5客户获取注册码/arduino使用教程.docx` | `P01`: Arduino IDE 2.x 与 ESP32 2.0.10 核心包版本锁定<br>`P02`: 目标开发板 ESP32 Dev Module 与 S1 芯片匹配<br>`P03`: 硬件切换黄灯与 CH340K 串口端口选定<br>`P04`: 固件编译上传与 Flash 烧录时序<br>`P05`: 115200 串口监视器监听与向客服提取注册码规程 | **LANDED** |
...
| **E-FW-005** | 主控板基础外设与传感器通信例程固件 | `4例程资料/主控板例程/` | `P01`: S1/S3 基础引脚映射（GPIO 25 / GPIO 1）<br>`P02`: 蜂鸣器（GPIO 9）与 WS2812B 炫彩灯驱动<br>`P03`: ESP32-S3 BLE GATT 蓝牙服务与特征值广播<br>`P04`: AS5600 磁编码器 I2C 轮询读取与角度解析<br>`P05`: MT6701 高速 SPI 与 I2C 双协议解码实现 | **LANDED** |
| **E-FW-006** | 双路无刷电机FOC控制与电流采样例程固件 | `4例程资料/主控板加小电流板/` | `P01`: 30kHz 开环速度与位置控制算法及 12.6V 基准<br>`P02`: INA199 相电流差分采样与一阶低通滤波<br>`P03`: Mode 4 力矩闭环模式跨板串口控制实现<br>`P04`: Mode 1 (速度) 与 Mode 2 (位置) 模式控制实现<br>`P05`: UDP 端口 2317 无线遥测与上位机接口 | **LANDED** |
| **E-FW-007** | 舵机扩展板PCA9685与MPU6050驱动例程固件 | `4例程资料/主控板加舵机板/` | `P01`: PCA9685 16通道 12-bit PWM 舵机驱动与占空比控制<br>`P02`: MPU-6050 六轴惯导姿态角与角速度实时读取 | **LANDED** |
