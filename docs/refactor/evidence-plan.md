...
| **E-simulation-PhysX闭环约束动力学配置** | **PhysX 闭环约束动力学原理与配置** | `SRC-DOC-002`, `SRC-PY-002` | 明确 NVIDIA PhysX 约束求解器对非树状闭环副的处理机制及 `excludeFromArticulation` 关键性 | P01: 笛卡尔约束与 Articulation 求解器隔离<br>P02: 闭环转动副参数配置规范（零摩擦/无驱动/无碰撞） | `闭环恢复架构与产物映射.md` | **CREATE** |
