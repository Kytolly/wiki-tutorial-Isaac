...
> 本知识库采用"按任务自定"分级（`intro → setup → train → build`），并同步维护 `_Sidebar` 导航、`_META` 状态和每页互链。
...
| 分级 | 目标 | 主要页面 | 完成标准 |
|------|------|----------|----------|
| **intro 概念启蒙** | 建立心智模型 | Isaac-Sim是什么、Isaac-Lab是什么 | 说清两框架是什么、彼此关系 |
| **setup 环境与 Demo** | 装好并跑通 | 安装与环境配置、运行第一个Demo、学习路线与下一步 | 装好环境、跑通官方示例 |
| **train 训练入门** | 完成最小训练 | 强化学习核心概念、Isaac-Lab项目结构与训练流程、第一个训练任务 | 理解 RL、独立跑一次训练并回放 |
| **build 自建环境进阶**（规划中） | 自建环境与调试 | 待生成 | 自建环境、修改 reward/observation |

## 学习顺序（推荐）

1. [[Isaac-Sim是什么]] → 2. [[Isaac-Lab是什么]] → 3. [[安装与环境配置]] → 4. [[运行第一个Demo]] → 5. [[学习路线与下一步]] → 6. [[强化学习核心概念]] → 7. [[Isaac-Lab项目结构与训练流程]] → 8. [[第一个训练任务]]
