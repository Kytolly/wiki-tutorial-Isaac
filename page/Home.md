...
| **robot 机器狗实战** | 用真实四轮足机器狗走通全流程 | 硬件与接口、模型与URDF、导入Isaac-Lab、运动任务设计、训练与调参、策略导出与真机部署、sim2real对齐与安全、常见问题 | 从 URDF 自建到真机部署走通一个真实机器人 |
...
> **机器狗实战路线**（用 StackForce 四轮足机器狗走通全流程）：[[硬件与接口]] → [[模型与URDF]] → [[导入Isaac-Lab]] → [[运动任务设计]] → [[训练与调参]] → [[策略导出与真机部署]] → [[sim2real对齐与安全]] → [[常见问题]]。
