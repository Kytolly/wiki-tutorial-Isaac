...
**Sim Time（仿真时间）**是物理世界内部认为过去了多久。
那么每执行一次 Physics Step：

$$ t_{sim} \leftarrow t_{sim}+\frac{1}{120} $$

也就是：

$$ \Delta t_{physics}=0.00833s $$

所以：物理仿真并不天然要求实时运行。这对强化学习尤其重要，因为我们通常希望：

$$ \text{simulation speed} \gg \text{real time} $$

例如现实 1 秒钟，GPU 可以模拟机器人世界里的 20 秒。`Physics Step` 才是物理仿真世界真正向前走一步，可以把：`Physics Step`理解为：根据当前状态 + 外力 + actuator + 碰撞约束，计算 \(\Delta t\) 之后的物理世界。

为什么图里一个 `App Update` 有两个 `Physics Step`？也就是说：

$$ physics\_dt=0.00833s=\frac1{120}s $$

但是一次 `App Update` 覆盖：

$$ 2\times0.00833=0.01667s $$
...
$$ f_{physics}=120Hz \\ f_{app}=60Hz $$

这意味着物理模拟两次，才更新一次应用/渲染。物理通常需要更高的更新频率才能保持稳定，根本没必要每一次 `physics step` 都渲染一张图片。

为什么控制器不需要每个 Physics Step 都计算一次？这是机器人仿真里特别重要的 multi-rate simulation（多速率仿真）。
假设：
```
Physics       120 Hz
RL Policy      60 Hz
...
图里的 Pre-Render / Rendering / Post-Render 是另一条流水线, 它属于渲染 pipeline。机器人从 \( q_t \) 变成：\( q_{t+\Delta t} \)，renderer 必须知道新的 link transform 才能生成正确的新画面。
```text
base
 └─ cart
      └─ pole
```
所以在无头模式下依旧可以照常训练。这也解释了为什么 GUI 卡不代表 Physics 卡。

`sleep`的设计目标实时仿真。 假设目标的实时帧率为 \( 60Hz \)，那么一帧应该花：\( T=\frac1{60}=16.67ms \)。但是你的电脑很强，只用了 10ms，那么剩下的 `sleep` 时间用于对齐实时目标的时间。但是 RL 训练通常恰恰不想 Sleep。强化学习的时候我们的目标通常不是：模拟 10 秒机器人运动真的等 10 秒。而是越快越好。
定义 Real-Time Factor：

$$ RTF=\frac{\Delta t_{sim}}{\Delta t_{real}} $$
