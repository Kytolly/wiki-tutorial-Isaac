...
```text
./isaaclab.sh --new 生成模板
      → pip install -e source/<项目> 安装并注册
      → 改 env_cfg 调奖励/环境数
      → train.py --task <你的任务名> 训练
      → play.py 回放 → 迭代改进
```
...
- 跑通本页，你就完成了 L1 的"独立完成最小训练任务"目标。

## 下一步

- 上一页：[[L1-Isaac-Lab项目结构与训练流程]]
- 下一页：返回 [[Home]]（L2 进阶规划中）
- 返回：[[Home]]
