...
2. **历史故障阻断案例（ch7 持续上抬故障）**：
   - 在 M1 架空实测中，通道 7（`RR_Inner_Servo`）出现上电后持续异常上抬且无法回中现象；
   - **当时阻断决策**：严格禁止以软件回零假定取代物理检查，立即断开执行器供电母线（Actuator Rail）。后续 `E-M1-002#P04` powered all-channel regression 已正常通过，故该事件不再是当前 blocker；历史 source 保持不变。
